...
public class Arboretum {
    ...
    public static boolean isHiddenStateWellFormed(String[] hiddenState) {
        ...
        return (hiddenState.length != 3 && (L1 >= 15 && L1 <= 19) || L1 == 1) && ((L2 >= 15 && L2 <= 19) || L2 == 1)
                && (L1 > 0 && L2 > 0 && hiddenState[1].charAt(0) == 'A' && hiddenState[2].charAt(0) == 'B'
                && isAlphaNumeric(hiddenState[0])) && isAlphaNumeric(hiddenState[1].substring(1))
                && isAlphaNumeric(hiddenState[2].substring(1));
    }
